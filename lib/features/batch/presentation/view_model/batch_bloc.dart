...
class BatchBloc extends Bloc<BatchEvent, BatchState> {
  ...
  Future<void> _onLoadBatches(
      LoadBatches event, Emitter<BatchState> emit) async {
        ...
        emit(state.copyWith(isLoading: false,batches: r))
        );
      }
  ...
  Future<void> _onAddBatch(AddBatch event, Emitter<BatchState> emit) async {
        ...
        var result = await _createBatchUsecase.call(CreateBatchParams(batchName: event.batchName));
        result.fold(
          (l)=> //in case of failure
        emit(state.copyWith(isLoading: false,error: l.toString())),
        (r)=>//in case of success
        ...
        //database bata tanera feri load batches wala function recall garxa ra page refresh hunxa
        add(LoadBatches());
  ...
  Future<void> _onDeleteBatch(
      DeleteBatch event, Emitter<BatchState> emit) async {
        ...
        var result = await _deleteBatchUsecase.call(DeleteBatchParams(event.batchId));
        result.fold(
          (l)=> //in case of failure
        emit(state.copyWith(isLoading: false,error: l.toString())),
        (r)=>//in case of success
